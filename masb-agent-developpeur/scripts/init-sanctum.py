...
SKILL_NAME = "masb-agent-developpeur"
...
def main():
    ...
    # Post-substitution validation: scan output files for remaining {placeholder} patterns
    remaining_placeholders = {}
    for md_file in sorted(sanctum_path.glob("*.md")):
        content = md_file.read_text()
        found = sorted(set(re.findall(r"\{[a-zA-Z][a-zA-Z0-9_. -]+\}", content)))
        if found:
            remaining_placeholders[md_file.name] = found
    if remaining_placeholders:
        print()
        print("  WARNING: Unresolved placeholders found in sanctum files:")
        for fname, placeholders in sorted(remaining_placeholders.items()):
            print(f"    {fname}: {', '.join(placeholders)}")
        print("  These should be resolved during First Breath or removed from templates.")
    else:
        print("  No unresolved placeholders — all variables substituted successfully.")
