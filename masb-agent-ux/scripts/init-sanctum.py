...
SKILL_ONLY_FILES = {"prompt-quality-canon.md", ".memlog.md"}

TEMPLATE_FILES = [
    "INDEX-template.md", "PERSONA-template.md", "CREED-template.md",
    "BOND-template.md", "MEMORY-template.md", "CAPABILITIES-template.md"
]
...
def generate_capabilities_md(capabilities: list[dict], evolvable: bool) -> str:
    ...
    lines.extend([
        ...
        "## Tools",
        "",
        "Prefer crafting your own tools over external APIs.",
        "",
        "### User-Provided Tools",
        "",
...
def build_report(project_root: str, skill_path: str, sanctum_path: str,
                 output: dict) -> dict:
    return {
        "status": "ok",
        "action": "scaffold",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="First Breath — Deterministic sanctum scaffolding for masb-agent-ux",
    )
    parser.add_argument(
        "project_root",
    ...
    parser.add_argument(
        "skill_path",
        help="Path to the skill directory (where SKILL.md, references/, assets/ live)",
    )
    ...
    if sanctum_path.exists():
        report = build_report(str(project_root), str(skill_path), str(sanctum_path), {
            "already_exists": True,
            "message": f"Sanctum already exists at {sanctum_path}. Skipping.",
        })
